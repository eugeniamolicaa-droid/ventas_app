...
# =========================
    ...
    with tab1:
        ...
        if df_productos_admin.empty:
            ...
            st.caption("Tocá el nombre de un producto para ver sus variantes. El resumen se edita desde el botón ✏️ del producto.")

            for idx, resumen in resumen_stock.iterrows():
                categoria_resumen = resumen.get("categoria") or ""
                nombre_resumen = resumen.get("nombre") or ""
                producto_variantes = df_productos_admin[
                    (df_productos_admin["categoria"].fillna("") == categoria_resumen)
                    & (df_productos_admin["nombre"].fillna("") == nombre_resumen)
                ].copy()
                ...
                with st.expander(
                    ...
                    with st.form(f"form_resumen_{producto_key}"):
                        ...
                        if guardar_resumen:
                            ids_producto = producto_variantes["id"].astype(int).tolist()
