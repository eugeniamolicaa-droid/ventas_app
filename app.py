...
def cargar_resumen_categorias():
    ...
    return pd.read_sql(
        """
        SELECT
            COALESCE(NULLIF(TRIM(categoria), ''), 'Sin categoría') AS categoria_mostrar,
            COALESCE(TRIM(categoria), '') AS categoria,
        ...
        FROM productos
        GROUP BY COALESCE(NULLIF(TRIM(categoria), ''), 'Sin categoría'), COALESCE(TRIM(categoria), '')
    ...
    return pd.read_sql(
        text("""
            SELECT
                ...
                COUNT(*) AS variantes
            FROM productos
            WHERE COALESCE(TRIM(categoria), '') = :categoria
            GROUP BY COALESCE(TRIM(categoria), ''), nombre
            ORDER BY nombre
        """),
        engine,
...
if menu == "🛒 INICIAR VENTA":
    ...
    df_productos["categoria_mostrar"] = df_productos["categoria"].apply(categoria_limpia)

    categorias_disponibles = sorted(df_productos["categoria_mostrar"].drop_duplicates().tolist())
    ...
    if categoria_seleccionada != "Todas":
        df_filtrado = df_filtrado[df_filtrado["categoria_mostrar"] == categoria_seleccionada]

    if categoria_seleccionada != "Todas" and not df_filtrado.empty:
        stock_categoria = int(df_filtrado["stock"].fillna(0).sum())
        total_costo_categoria = float((df_filtrado["stock"].fillna(0) * df_filtrado["costo"].fillna(0)).sum())
        total_venta_categoria = float((df_filtrado["stock"].fillna(0) * df_filtrado["precio"].fillna(0)).sum())
        mc1, mc2, mc3 = st.columns(3)
        with mc1:
            st.metric("Stock total categoría", stock_categoria)
        with mc2:
            st.metric("Total costo", f"${total_costo_categoria:,.0f}")
        with mc3:
            st.metric("Total venta", f"${total_venta_categoria:,.0f}")

    total_resultados = len(df_filtrado)
    df_filtrado = df_filtrado.head(48)
    st.write("")
        ...
        if resumen_categorias.empty:
            ...
            for idx_cat, cat in resumen_categorias.iterrows():
                ...
                with st.expander(
                    ...
                    f"Costo: ${total_costo_cat:,.0f} | Venta: ${total_venta_cat:,.0f} | {rango_precios}"
                ):
                    cm1, cm2, cm3 = st.columns(3)
                    with cm1:
                        st.metric("Stock total", int(cat["stock_total"] or 0))
                    with cm2:
                        st.metric("Dinero en costo", f"${total_costo_cat:,.0f}")
                    with cm3:
                        st.metric("Dinero en venta", f"${total_venta_cat:,.0f}")
                    ...
                    if productos_categoria.empty:
                        ...
                        for idx_prod, resumen in productos_categoria.iterrows():
                            ...
                            with st.expander(
                                ...
                                f"Costo: ${total_costo_prod:,.0f} | Venta: ${total_venta_prod:,.0f} | "
                                ...
                                with pm2:
                                    st.metric("Total costo", f"${total_costo_prod:,.0f}")
                                with pm3:
                                    st.metric("Total venta", f"${total_venta_prod:,.0f}")
                                        ...
                                        with engine.begin() as conn:
                                            conn.execute(text("""
                                                ...
                                                WHERE nombre = :old_nom
                                                  AND COALESCE(categoria, '') = :old_cat
                                            """), {
