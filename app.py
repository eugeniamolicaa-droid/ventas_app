...
def normalizar_busqueda(valor) -> str:
    """Normaliza texto para buscar productos por modelo, color o variante."""
    texto = str(valor or "").strip().lower()
    texto = unicodedata.normalize("NFKD", texto)
    texto = "".join(ch for ch in texto if not unicodedata.combining(ch))
    texto = texto.replace("pro-max", "pro max").replace("promax", "pro max")
    texto = texto.replace("x/xs", "x xs").replace("x-xs", "x xs")
    texto = re.sub(r"[^a-z0-9]+", " ", texto)
    return " ".join(texto.split())


def fila_coincide_busqueda_producto(row, busqueda: str) -> bool:
    """Hace que '15 rosa' encuentre solo fundas que tengan modelo 15 y color rosa."""
    consulta = normalizar_busqueda(busqueda)
    if not consulta:
        return True

    texto_producto = normalizar_busqueda(
        f"{row.get('nombre', '')} {row.get('variante', '')} {row.get('categoria', '')}"
    )

    return all(token in texto_producto for token in consulta.split())
...
def orden_producto_fila(row) -> str:
    ...
    return f"{texto_simple}|{variante_rank:02d}|{texto_orden_natural(nombre)}|{texto_orden_natural(variante)}"
...
def crear_pdf_fin_dia(resumen, ventas_detalle):
    ...
    y -= 0.6 * cm
    pdf.drawString(2 * cm, y, f"Total stickers vendidos: {int(resumen.get('stickers_cantidad', 0) or 0)}")

    y -= 0.6 * cm
    pdf.drawString(2 * cm, y, f"Monto total stickers: ${float(resumen.get('stickers_monto', 0) or 0):,.0f}")
    ...
    if detalle_productos.empty:
        ...
        total_stickers = int(stickers["cantidad_total"].sum()) if not stickers.empty else int(resumen.get("stickers_cantidad", 0) or 0)
        monto_stickers = float(stickers["monto_total"].sum()) if not stickers.empty else float(resumen.get("stickers_monto", 0) or 0)
...
if menu == "🛒 INICIAR VENTA":
    ...
    st.caption("Elegí categoría y vendé con tarjetas visuales. En Fundas aparece buscador por modelo y color para encontrar todo más rápido.")
    ...
    es_categoria_fundas = (
        ...
        and "funda" in categoria_slug(categoria_seleccionada)
    )

    if es_categoria_fundas:
        st.markdown("#### 🔎 Buscar funda")
        buscador_fundas = st.text_input(
            "Modelo o color",
            placeholder="Ej: 11, 15 rosa, 13 pro max negro",
            key="buscador_fundas_ventas"
        )

        if buscador_fundas.strip():
            df_filtrado = df_filtrado[
                df_filtrado.apply(lambda r: fila_coincide_busqueda_producto(r, buscador_fundas), axis=1)
            ]
            st.caption(f"Filtro activo: {buscador_fundas}")
        ...
        if resumen_categorias.empty:
            ...
            for idx_cat, cat in resumen_categorias.iterrows():
                ...
                with st.expander(
                    ...
                    st.divider()
                    productos_categoria = cargar_productos_por_categoria(categoria_real)
                    ...
                    if productos_categoria.empty:
                        ...
                        for idx_prod, resumen in productos_categoria.iterrows():
                            ...
                            with st.expander(
                                ...
                                st.divider()
                                producto_variantes = cargar_variantes_producto(nombre_resumen, categoria_resumen)
    ...
    try:
        ...
        if cobros.empty:
            ...
            if cobros_hoy.empty:
                ...
                grupos_hoy_para_stickers = cobros_hoy["venta_grupo"].dropna().tolist()
                stickers_cantidad_dia = 0
                stickers_monto_dia = 0.0

                if grupos_hoy_para_stickers:
                    detalle_stickers_dia = pd.read_sql(text("""
                        SELECT
                            COALESCE(SUM(cantidad), 0) AS cantidad_total,
                            COALESCE(SUM(total), 0) AS monto_total
                        FROM ventas
                        WHERE COALESCE(es_sticker, FALSE) = TRUE
                          AND venta_grupo = ANY(:grupos)
                    """), engine, params={"grupos": grupos_hoy_para_stickers})

                    if not detalle_stickers_dia.empty:
                        stickers_cantidad_dia = int(detalle_stickers_dia.iloc[0]["cantidad_total"] or 0)
                        stickers_monto_dia = float(detalle_stickers_dia.iloc[0]["monto_total"] or 0)

                s1, s2 = st.columns(2)
                with s1:
                    st.metric("Stickers vendidos hoy", stickers_cantidad_dia)
                with s2:
                    st.metric("Total stickers hoy", f"${stickers_monto_dia:,.0f}")
                ...
                resumen_fin_dia = {
                    ...
                    "stickers_cantidad": stickers_cantidad_dia,
                    "stickers_monto": stickers_monto_dia
                }
                ...
                with col_nuevo_dia:
                    ...
                    st.caption("Borra ventas y cobros de TODOS los usuarios. No devuelve stock.")
                    ...
                    if st.button(
                        "🌅 NUEVO DÍA - BORRAR VENTAS",
                        type="primary",
                        use_container_width=True,
                        key="nuevo_dia_confirmado"
                    ):
                        if not st.session_state.get("fin_dia_pdf_descargado", False):
                            st.error("❌ Primero descargá el PDF de fin del día. Después podés borrar las ventas.")

                        elif not password_nuevo_dia:
                            ...
                            if not admin_ok:
                                ...
                                with engine.begin() as conn:
                                    ...
                                    conn.execute(text("DELETE FROM cobros"))
                                ...
                                st.success("✅ Nuevo día iniciado. Se borraron ventas y cobros de todos los usuarios sin devolver stock.")
                                st.rerun()
