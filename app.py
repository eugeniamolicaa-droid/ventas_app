...
def categoria_titulo_desde_grupo(valores):
    ...
    elegido = sorted(limpios, key=lambda x: (len(x), x.lower()))[0].strip()
    elegido = elegido.title().replace("Iphone", "iPhone")
    return elegido




def subcategoria_limpia(subcategoria) -> str:
    texto = str(subcategoria or "").strip()
    return texto


def subcategoria_slug(subcategoria) -> str:
    texto = subcategoria_limpia(subcategoria).lower()
    texto = unicodedata.normalize("NFKD", texto)
    texto = "".join(ch for ch in texto if not unicodedata.combining(ch))
    return " ".join(texto.split())


def etiqueta_categoria_subcategoria(categoria, subcategoria) -> str:
    categoria = categoria_limpia(categoria)
    subcategoria = subcategoria_limpia(subcategoria)
    if subcategoria:
        return f"{categoria} / {subcategoria}"
    return categoria


def detectar_modelo_iphone(texto) -> tuple:
    """Devuelve nombre ordenado y ranking para fundas iPhone. X/XS queda al final."""
    t = str(texto or "").lower()
    t = unicodedata.normalize("NFKD", t)
    t = "".join(ch for ch in t if not unicodedata.combining(ch))
    t = t.replace("pro-max", "pro max").replace("promax", "pro max")
    t = re.sub(r"[^a-z0-9/ +]", " ", t)
    t = " ".join(t.split())

    # iPhone 11 al 17, en orden de familia y después por versión.
    for numero in range(11, 18):
        if re.search(rf"(?:iphone|iph|ip)?\s*{numero}(?!\d)", t):
            if "pro max" in t:
                return f"iPhone {numero} Pro Max", numero * 100 + 30
            if re.search(r"\bpro\b", t):
                return f"iPhone {numero} Pro", numero * 100 + 20
            if re.search(r"\bplus\b", t):
                return f"iPhone {numero} Plus", numero * 100 + 10
            if re.search(r"\bmini\b", t):
                return f"iPhone {numero} Mini", numero * 100 + 5
            return f"iPhone {numero}", numero * 100

    # X/XS pedido al final. Incluye X, XS y XS Max.
    if re.search(r"\b(xs\s*max|x\s*/\s*xs|x\s+xs|xs|iphone\s*x)\b", t):
        if "max" in t:
            return "iPhone XS Max", 9905
        return "iPhone X/XS", 9900

    return None, 9999


def es_categoria_fundas(categoria) -> bool:
    slug = categoria_slug(categoria)
    return "funda" in slug


def es_funda_iphone(row) -> bool:
    categoria = row.get("categoria", "") if hasattr(row, "get") else ""
    subcategoria = row.get("subcategoria", "") if hasattr(row, "get") else ""
    nombre = row.get("nombre", "") if hasattr(row, "get") else ""
    variante = row.get("variante", "") if hasattr(row, "get") else ""
    texto = f"{categoria} {subcategoria} {nombre} {variante}"
    modelo, _ = detectar_modelo_iphone(texto)
    return es_categoria_fundas(categoria) and (subcategoria_slug(subcategoria) == "iphone" or modelo is not None)


def orden_funda_iphone(row) -> str:
    nombre = str(row.get("nombre", "") or "")
    variante = str(row.get("variante", "") or "")
    modelo, modelo_rank = detectar_modelo_iphone(f"{nombre} {variante}")
    texto = texto_orden_natural(f"{nombre} {variante}")
    return f"00-fundas-iphone|{modelo_rank:05d}|{texto}"


def organizar_texto_funda_iphone(nombre, variante):
    modelo, _ = detectar_modelo_iphone(f"{nombre} {variante}")
    return modelo


def texto_orden_natural(valor) -> str:
    """Ordena textos mezclando palabras y números: iPhone 11, 12, 13... sin saltos raros."""
    texto = str(valor or "").strip().lower()
    texto = unicodedata.normalize("NFKD", texto)
    texto = "".join(ch for ch in texto if not unicodedata.combining(ch))
    texto = texto.replace("pro-max", "pro max").replace("promax", "pro max")
    texto = " ".join(texto.split())
    return re.sub(r"\d+", lambda m: f"{int(m.group()):05d}", texto)


def orden_producto_fila(row) -> str:
    if es_funda_iphone(row):
        return orden_funda_iphone(row)
    ...
    texto = texto_orden_natural(f"{nombre} {variante}")
    texto_simple = texto.replace("  ", " ")

    if "pro max" in texto_simple:
        variante_rank = 30
    elif "pro" in texto_simple:
        variante_rank = 20
    elif "plus" in texto_simple:
        variante_rank = 10
    else:
        variante_rank = 0

    return (
        f"{texto_orden_natural(categoria)}|{texto_orden_natural(subcategoria)}|"
        f"{texto_simple}|{variante_rank:02d}|{texto_orden_natural(nombre)}|{texto_orden_natural(variante)}"
    )


def ordenar_dataframe_productos(df):
    if df is None or df.empty:
        return df
    return (
        df.copy()
        .assign(_orden_producto=df.apply(orden_producto_fila, axis=1))
        .sort_values("_orden_producto")
        .drop(columns=["_orden_producto"])
    )
...
def render_category_chips(df_productos):
    ...
    df_tmp["subcategoria"] = df_tmp.get("subcategoria", "").fillna("")
    categorias = (
        df_tmp.apply(lambda r: etiqueta_categoria_subcategoria(r.get("categoria"), r.get("subcategoria")), axis=1)
        .drop_duplicates()
        .tolist()
    )
...
with engine.begin() as conn:
    ...
    conn.execute(text("ALTER TABLE productos ADD COLUMN IF NOT EXISTS subcategoria TEXT;"))
...
def organizar_fundas_iphone_en_bd():
    """Agrupa fundas iPhone dentro de Fundas / iPhone y normaliza el nombre del modelo."""
    df = pd.read_sql(
        """
        SELECT id, categoria, subcategoria, nombre, variante
        FROM productos
        WHERE LOWER(COALESCE(categoria, '')) LIKE '%funda%'
           OR LOWER(COALESCE(nombre, '')) LIKE '%iphone%'
           OR LOWER(COALESCE(variante, '')) LIKE '%iphone%'
        """,
        engine
    )
    ...
    for _, row in df.iterrows():
        modelo = organizar_texto_funda_iphone(row.get("nombre"), row.get("variante"))
        if not modelo:
            continue

        categoria_actual = categoria_slug(row.get("categoria"))
        texto_actual = texto_orden_natural(f"{row.get('categoria')} {row.get('nombre')} {row.get('variante')}")

        # Solo toca fundas o productos claramente cargados como iPhone en la categoría Fundas.
        if "funda" not in categoria_actual and "funda" not in texto_actual:
            continue
...
def cargar_productos():
    ...
    return pd.read_sql(
        """
        SELECT id, categoria, subcategoria, nombre, variante, precio, costo, stock, imagen
        FROM productos
        ORDER BY categoria, subcategoria, nombre, variante
        """,
        engine
    )
...
def cargar_resumen_categorias():
    ...
    return pd.read_sql(
        ...
        SELECT
            ...
            INITCAP(MIN(COALESCE(NULLIF(TRIM(subcategoria), ''), ''))) AS subcategoria_mostrar,
            COALESCE(LOWER(TRIM(subcategoria)), '') AS subcategoria,
        ...
        FROM productos
        GROUP BY COALESCE(LOWER(TRIM(categoria)), ''), COALESCE(LOWER(TRIM(subcategoria)), '')
        ORDER BY categoria_mostrar, subcategoria_mostrar
        """,
        engine
    )
...
@st.cache_data(ttl=120)
def cargar_productos_por_categoria(categoria, subcategoria=""):
    # Cuando abrís una categoría/subcategoría, recién ahí aparecen sus productos.
    return pd.read_sql(
        text("""
            SELECT
                COALESCE(LOWER(TRIM(categoria)), '') AS categoria,
            ...
            WHERE COALESCE(LOWER(TRIM(categoria)), '') = :categoria
              AND COALESCE(LOWER(TRIM(subcategoria)), '') = :subcategoria
            GROUP BY COALESCE(LOWER(TRIM(categoria)), ''), COALESCE(LOWER(TRIM(subcategoria)), ''), nombre
            ORDER BY nombre
        """),
        engine,
        params={"categoria": categoria or "", "subcategoria": subcategoria or ""}
    )


@st.cache_data(ttl=120)
def cargar_variantes_producto(nombre, categoria, subcategoria=""):
    # Solo trae las variantes cuando abrís un producto.
    return pd.read_sql(
        text("""
            SELECT id, categoria, subcategoria, nombre, variante, precio, costo, stock
            FROM productos
            WHERE nombre = :nombre
        ...
        """),
        engine,
        params={"nombre": nombre, "categoria": categoria or "", "subcategoria": subcategoria or ""}
    )
...
if menu == "🛒 INICIAR VENTA":
    ...
    st.caption("Para facturar cositas chicas sin cargarlas al stock. Entra al carrito y al reporte, pero no descuenta inventario.")

    with st.expander("➕ Agregar producto manual sin stock", expanded=False):
        with st.form("form_producto_manual_venta", clear_on_submit=True):
            col_m1, col_m2, col_m3 = st.columns([2.2, 1, 0.8])

            with col_m1:
                manual_nombre = st.text_input(
                    "Detalle",
                    placeholder="Ej: Cable, arreglo, diferencia, repuesto...",
                    key="manual_nombre_venta"
                )
            ...
            if agregar_manual:
                nombre_manual = (manual_nombre or "").strip()

                if not nombre_manual:
                    st.error("Escribí qué estás facturando")
                elif manual_precio <= 0:
                    st.error("El precio tiene que ser mayor a 0")
                else:
                    st.session_state["cart"].append({
                        "id": None,
                        "name": nombre_manual,
                        "variant": "Producto manual",
                        "price": float(manual_precio),
                        "cost": 0.0,
                        "qty": int(manual_qty),
                        "manual_sale": True
                    })
                    reset_pagos()
                    st.toast(f"✅ {nombre_manual} agregado sin tocar stock", icon="🧾")
                    st.rerun()
    ...
    df_productos["subcategoria"] = df_productos.get("subcategoria", "").fillna("")
    df_productos["categoria_key"] = df_productos["categoria"].apply(categoria_slug)
    df_productos["subcategoria_key"] = df_productos["subcategoria"].apply(subcategoria_slug)
    df_productos["categoria_label"] = df_productos.apply(
        lambda r: etiqueta_categoria_subcategoria(r.get("categoria"), r.get("subcategoria")),
        axis=1
    )

    categorias_df = (
        df_productos
        .groupby(["categoria_key", "subcategoria_key"], dropna=False)
        .agg(categoria_mostrar=("categoria_label", categoria_titulo_desde_grupo))
        .reset_index()
        .sort_values("categoria_mostrar")
    )
    categoria_label_to_key = {
        row["categoria_mostrar"]: (row["categoria_key"], row["subcategoria_key"])
    ...
    if categoria_seleccionada != "Todas":
        categoria_key_seleccionada, subcategoria_key_seleccionada = categoria_label_to_key.get(categoria_seleccionada, (None, None))
        df_filtrado = df_filtrado[
            (df_filtrado["categoria_key"] == categoria_key_seleccionada)
            & (df_filtrado["subcategoria_key"] == subcategoria_key_seleccionada)
        ]
    ...
    df_filtrado = ordenar_dataframe_productos(df_filtrado)
    ...
    if df_filtrado.empty:
        ...
        for idx, row in df_filtrado.reset_index(drop=True).iterrows():
            with cols[idx % 4]:
                ...
                categoria = etiqueta_categoria_subcategoria(row.get("categoria"), row.get("subcategoria"))
    ...
    if not cart:
        ...
        for i, item in enumerate(cart):
            ...
            with col1:
                ...
                manual_txt = " · Manual" if item.get("manual_sale") else ""
                st.write(f"**{item['name']}** {item.get('variant', '')} × {item['qty']}{promo_txt}{manual_txt}")
...
# =========================
    ...
    with tab1:
        st.subheader("Ordenador rápido de Fundas iPhone")
        st.caption("Agrupa todas las fundas de iPhone en Fundas / iPhone y las renombra por modelo: 11, 12, 13, 14, 15, 16, 17 y X/XS al final. No toca stock, precios, costo ni fotos.")

        if st.button("📱 Ordenar fundas iPhone", type="primary", use_container_width=True, key="btn_ordenar_fundas_iphone"):
            cantidad_organizada = organizar_fundas_iphone_en_bd()
            if cantidad_organizada:
                st.success(f"✅ Listo: {cantidad_organizada} funda(s) quedaron en Fundas / iPhone, ordenadas por modelo.")
            else:
                st.info("No encontré fundas iPhone para reorganizar.")
            st.rerun()
        ...
        with st.form("form_nuevo_producto_variantes", clear_on_submit=True):
            ...
            with col1:
                ...
                subcategoria = st.text_input("Subcategoría", placeholder="Ej: iPhone", key="nueva_subcategoria")
                        ...
                        with engine.begin() as conn:
                            for variante_nombre, variante_stock in variantes:
                                conn.execute(text("""
                                    ...
                                    (categoria, subcategoria, nombre, variante, precio, costo, stock, imagen)
                                    VALUES (:cat, :subcat, :nom, :var, :pre, :cos, :sto, :img)
                                """), {
                                    "cat": categoria,
                                    "subcat": subcategoria,
                                    "nom": nombre,
        ...
        if resumen_categorias.empty:
            ...
            for idx_cat, cat in resumen_categorias.iterrows():
                ...
                subcategoria_real = cat.get("subcategoria") or ""
                categoria_mostrar = etiqueta_categoria_subcategoria(cat.get("categoria_mostrar") or "Sin categoría", cat.get("subcategoria_mostrar") or "")
                categoria_key = f"cat_group_{idx_cat}_{abs(hash(str(categoria_real) + str(subcategoria_real))) % 100000}"
                ...
                with st.expander(
                    ...
                    st.divider()
                    productos_categoria = cargar_productos_por_categoria(categoria_real, subcategoria_real)
                    productos_categoria = ordenar_dataframe_productos(productos_categoria)
                    ...
                    if productos_categoria.empty:
                        ...
                        for idx_prod, resumen in productos_categoria.iterrows():
                            ...
                            subcategoria_resumen = resumen.get("subcategoria") or ""
                            nombre_resumen = resumen.get("nombre") or ""

                            producto_key = (
                                f"prod_group_{categoria_key}_{idx_prod}_"
                                f"{abs(hash(str(categoria_resumen) + str(subcategoria_resumen) + str(nombre_resumen))) % 100000}"
                            )
                            ...
                            with st.expander(
                                ...
                                st.divider()
                                producto_variantes = cargar_variantes_producto(nombre_resumen, categoria_resumen, subcategoria_resumen)
                                producto_variantes = ordenar_dataframe_productos(producto_variantes)
                                ...
                                with st.form(f"form_resumen_{producto_key}"):
                                    ...
                                    with col_r1:
                                        ...
                                        nueva_subcategoria_grupo = st.text_input(
                                            "Subcategoría",
                                            value=subcategoria_resumen,
                                            key=f"res_subcategoria_{producto_key}"
                                        )
        ...
        if "edit_product_id" in st.session_state:
            ...
            with st.form("form_editar_producto"):
                ...
                with col1:
                    ...
                    e_subcategoria = st.text_input("Subcategoría", value=prod.get("subcategoria") or "", key="edit_subcategoria")
