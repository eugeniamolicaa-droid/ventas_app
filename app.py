...
def cargar_productos():
    # Consulta liviana para venta: NO trae imagen. Esto acelera muchísimo la pantalla principal.
    return pd.read_sql(
        """
...
@st.cache_data(ttl=120)
def cargar_resumen_productos():
    # Resumen liviano para Admin: tampoco trae imágenes ni variantes completas.
    return pd.read_sql(
        """
        SELECT
            ...
            COUNT(*) AS variantes
        FROM productos
        GROUP BY COALESCE(categoria, ''), nombre
        ORDER BY nombre
        """,
        engine
    )
...
if menu == "🛒 Agregar Producto":
    ...
    df_filtrado = df_filtrado.head(36)
    ...
    if df_filtrado.empty:
        ...
        for idx, row in df_filtrado.reset_index(drop=True).iterrows():
            with cols[idx % 4]:
                ...
                # Modo turbo: no renderizamos fotos en venta. La pantalla responde mucho más rápido.
                st.markdown(f"""
...
# =========================
    ...
    with tab1:
        ...
        resumen_stock = cargar_resumen_productos()

        if resumen_stock.empty:
            st.info("No hay productos registrados")
        else:
            st.markdown("### 📦 Resumen por producto")
            st.caption("Modo turbo: primero carga solo el resumen. Las variantes aparecen recién cuando abrís un producto.")

            for idx, resumen in resumen_stock.iterrows():
                categoria_resumen = resumen.get("categoria") or ""
                nombre_resumen = resumen.get("nombre") or ""

                producto_key = f"prod_group_{idx}_{abs(hash(str(categoria_resumen) + str(nombre_resumen))) % 100000}"
