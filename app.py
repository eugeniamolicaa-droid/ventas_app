...
def cargar_resumen_categorias():
    ...
    return pd.read_sql(
        """
        SELECT
            COALESCE(NULLIF(categoria, ''), 'Sin categoría') AS categoria_mostrar,
        ...
        FROM productos
        GROUP BY COALESCE(NULLIF(categoria, ''), 'Sin categoría'), COALESCE(categoria, '')
        ...
        if resumen_categorias.empty:
            ...
            for idx_cat, cat in resumen_categorias.iterrows():
                ...
                with st.expander(
                    ...
                    f"{int(cat['variantes'] or 0)} variantes | Stock: {int(cat['stock_total'] or 0)} | {rango_precios}"
                ):
